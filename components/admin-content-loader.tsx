...
import { useAdminContent } from "@/lib/hooks/use-admin-content";
import { getContent, getContentWithHtml } from "@/lib/hooks/use-page-content";
import { Loader2 } from "lucide-react";

interface AdminContentLoaderProps {
  children: (data: { content: ReturnType<typeof useAdminContent>["content"] }) => React.ReactNode;
}

export function AdminContentLoader({ children }: AdminContentLoaderProps) {
  const { content, loading, error } = useAdminContent("es");
