...
import { useTestContent } from "@/lib/hooks/use-test-content";
import { getContent, getContentWithHtml } from "@/lib/hooks/use-page-content";
import { Loader2 } from "lucide-react";

interface TestContentLoaderProps {
  children: (data: { content: ReturnType<typeof useTestContent>["content"] }) => React.ReactNode;
}

export function TestContentLoader({ children }: TestContentLoaderProps) {
  const { content, loading, error } = useTestContent("es");
