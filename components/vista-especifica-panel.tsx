...
// Componente separado que solo muestra ÁREA COMPLETA (sin cubo 2x2 ni botón volver)
export function AreaCompletaPanel({ areaIndex, subAreaIndex, answers, darkMode = false }: Omit<VistaEspecificaPanelProps, 'onBack'>) {
  ...
  return (
    <div className={`border rounded-lg p-6 mb-0 ${darkMode ? 'border-white/20 bg-white/5' : 'border-gray-300 bg-white'}`} style={{ marginTop: '30px' }}>
      ...
      {/* Header Superior: NUEVA ESTRUCTURA según imagen */}
      <div className="grid grid-cols-1 lg:grid-cols-[auto_auto_auto_1fr] gap-6 mb-4 items-start">
        ...
        {/* Columna 2: DESGLOSE POR SUB ÁREAS - CON CUADRADOS GRADIENTE */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Columna 1: PROPÓSITO DEL ÁREA */}
        <div>
          <h4 className={`text-sm font-bold mb-2.5 flex items-center gap-2 ${darkMode ? 'text-white' : 'text-gray-900'}`}>
            <span className="text-base" style={{ color: areaColor }}>🎯</span>
            <span style={{ color: areaColor }}>PROPÓSITO DEL ÁREA:</span>
          </h4>
          {contenidoDisponible && (
        ...
        {/* Columna 2: PRÓXIMOS PASOS RECOMENDADOS */}
        <div>
          <h4 className={`text-sm font-bold mb-2.5 flex items-center gap-2 ${darkMode ? 'text-white' : 'text-gray-900'}`}>
            <span className="text-base" style={{ color: areaColor }}>⚡</span>
            <span style={{ color: areaColor }}>PRÓXIMOS PASOS RECOMENDADOS:</span>
          </h4>
